...
?>
                <div class="chat-main-box">
					...
					<li>
						<a href="<?php echo site_url('patient/message/message_read/' . $row['message_thread_code']);?>" class="<?php if(isset($current_message_thread_code) && $current_message_thread_code == $row['message_thread_code']) echo 'active' ;?>">
                            <img src="<?php echo $this->crud_model->get_user_image_url($user_to_show_id); ;?>" class="img-circle" draggable="false"/> 
                            <span>
                            <?php 
                            $user_details = $this->db->get_where('users', array('user_id' => $user_to_show_id))->row_array() ;
                            $hw_details = $this->db->get_where('health_worker', array('health_worker_id' => $user_to_show_id))->row_array() ;
                            if ($user_details) {
                                echo $user_details['fname'] .'  '. $user_details['lname'];
                            }elseif ($hw_details) {
                               echo $hw_details['fname'] .'  '. $hw_details['lname'];
                            }
                    ...
                    <div class="chat-right-aside"> 
                        <div class="chat-main-header">
							...
							<?php 
							$selectUser = $this->db->get_where('users', array('user_id' => $message_thread_details['receiver']))->row();
                            $selectHw = $this->db->get_where('health_worker', array('health_worker_id' => $message_thread_details['receiver']))->row();
                            if ($selectUser) {
                                echo $selectUser->fname . ' ' .$selectUser->lname;
                            }elseif ($selectHw) {
                                 echo $selectHw->fname . ' ' .$selectHw->lname;
                            }
							...
							?>
								...
								<div class="btn-group pull-right">
									...
									<ul class="dropdown-menu">
										<li><a href="<?php echo base_url(); ?>patient/message/message_new">New Message</a> 
										</li>
									</ul>
								</div><br>
                        	</div>
                         ...
                         <div class="chat-box">
                             <ul class="chat-list slimscroll" style="overflow: hidden;" tabindex="5005">   
                                ...
                                $sender_details = $this->crud_model->get_all_users($row['sender'])->row_array();
								...
								<li class="odd">
                                    ...
                                    <div class="chat-body">
                                        <div class="chat-text" style="border:-moz-border-radius: 1em 4em 1em 4em;border-radius: 1em 4em 1em 4em;display:inline-block;">
                                            ...
                                            <?php 
                                                $selectUser = $this->db->get_where('users', array('user_id' => $sender_id))->row();
                                                echo $selectUser->fname . ' ' .$selectUser->lname;
                                            ?>
                                            ...
                                            <i><?php echo date('H, s i', $row['timestamp']);?></i>
                                ...
                                <li>
                                    ...
                                    <div class="chat-body">
                                        ...
                                        <h4>
                                            ...
                                            <?php 
                                                $selectUser = $this->db->get_where('users', array('user_id' => $sender_id))->row();
                                                $selectHw = $this->db->get_where('health_worker', array('health_worker_id' => $sender_id))->row();
                                                if ($selectUser) {
                                                   echo $selectUser->fname . ' ' .$selectUser->lname;
                                                }elseif ($selectHw) {
                                                   echo $selectHw->fname . ' ' .$selectHw->lname;
...
<?php
   if ($this->session->userdata('health_worker_id')) { 
     $message_thread_details = $this->db->get_where('message_thread', array('message_thread_code' => $current_message_thread_code))->row_array();
    $first_sender = $message_thread_details['sender'];
    ?>
        <div class="chat-main-box">
                    ...
                    <div class="chat-left-aside">
                        ...
                        <div class="chat-left-inner">
                            ...
                            <?php 
                                $current_user = $this->session->userdata('health_worker_id');
                                $this->db->where('sender', $current_user);
                                $this->db->or_where('receiver', $current_user);
                                $message_threads = $this->db->get('message_thread')->result_array();
                                foreach ($message_threads as $key => $row)  :     
                    ...
                    <li>
                        <a href="<?php echo site_url('patient/message/message_read/' . $row['message_thread_code']);?>" class="<?php if(isset($current_message_thread_code) && $current_message_thread_code == $row['message_thread_code']) echo 'active' ;?>">
                            ...
                            <span>
                            <?php 
                            $user_details = $this->db->get_where('health_worker', array('health_worker_id' => $user_to_show_id))->row_array() ;
                            if ($user_details) {
                               echo $user_details['fname'] .'  '. $user_details['lname'];
                            }
                    ...
                    <div class="chat-right-aside"> 
                        <div class="chat-main-header">
                            ...
                            You are chatting with : 
                            <?php 
                            $selectUser = $this->db->get_where('health_worker', array('health_worker_id' => $message_thread_details['receiver']))->row();
                            echo $selectUser->fname . ' ' .$selectUser->lname;
                            ?>
                         ...
                         <div class="chat-box">
                             <ul class="chat-list slimscroll" style="overflow: hidden;" tabindex="5005">   
                                ...
                                $sender_details = $this->crud_model->get_all_health_workers($row['sender'])->row_array();
                                $sender_id = $row['sender'];
                                $data['message_thread_code'] = $current_message_thread_code;
                                ...
                                <?php if($sender_id == $this->session->userdata('health_worker_id')) : ?>
                                <li class="odd">
                                    <div class="chat-image"> <img src="" class="img-circle" draggable="false"/> </div>
                                    <div class="chat-body">
                                        <div class="chat-text" style="border:-moz-border-radius: 1em 4em 1em 4em;border-radius: 1em 4em 1em 4em;display:inline-block;">
                                            ...
                                            <?php 
                                                $selectUser = $this->db->get_where('health_worker', array('health_worker_id' => $sender_id))->row();
                                                echo $selectUser->fname . ' ' .$selectUser->lname;
                                            ?>
                        ...
                        var message = $("textarea#message").val();
                         if (message == "") {
                            ...
                            $(document).ready(function() {
                                $.toast({
                                    heading: 'Error !!!',
                                    text: '<?php echo ('no_options_can_be_blank_and_there_has_to_be_atleast_one_answer'); ?>',
                            ...
                            jQuery.ajax({
                            type: "POST",
                            url: "<?php echo base_url(); ?>" + "patient/message/send_reply/<?php echo $current_message_thread_code?>",
                            dataType: 'json',
